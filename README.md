...
| 7–9 | ✅ Завершены | LangGraph workflow, UI, synthetic data, happy-path кейс INC-000123 |
| 10 | 🔜 В работе | Full evaluation по метрикам M-01…M-09, LLM-as-a-judge |
| **10.1** | **🚧 В работе** | **CI/CD Stabilization**: Изоляция тестов, мокирование Ollama/ChromaDB, env-переменные |
| 11+ | 📅 Planned | Расширение Golden Dataset, оптимизация latency, multi-language support |

## 🚧 Известные ограничения и TODO (CI/CD)
> **Контекст:** Текущий пайплайн GitHub Actions требует рефакторинга для обеспечения детерминированности и полной изоляции от внешних сервисов (Ollama, ChromaDB). Бейдж CI временно отражает статус `failing` до завершения стабилизации.

**Ключевые задачи (TODO):**
- [ ] **Изоляция LLM/RAG:** Настройка моков для `langchain-ollama` и `chromadb` в `conftest.py` (предотвращение реальных сетевых вызовов на этапе импорта).
- [ ] **Переменные окружения:** Инъекция mock-значений (`OLLAMA_HOST`, `CHROMA_PATH`, `THRESHOLD_*`) в `.github/workflows/ci.yml` для успешного прохождения Pydantic-валидации.
- [ ] **Оптимизация установки:** Добавление кэширования `pip` (или переход на `uv`) для предотвращения ошибок резолвинга зависимостей (backtracking).
- [ ] **Данные в CI:** Использование in-memory хранилищ для тестового Golden Dataset вместо обращения к файловой системе или БД.


## 🤝 Вклад и контакты

Проект создан в образовательных целях для демонстрации компетенций AI Engineer.
